 ...
 * @see 	 
 */
public class CheckTabIsFinished extends AbstractCheckListener {

	public CheckTabIsFinished() {
		// TODO Auto-generated constructor stub

	}

	public CheckTabIsFinished(IConnection connection) {
		super(connection);
	...
	public Object action(String action, Object... args) throws HDException {
		...
		judgeMapListValue(mapParas, PDAWorkOrderInfoConfig.class, false);
		//判断导航栏菜单
		if(listPDAConfigInfo!=null&&listPDAConfigInfo.size()>0)
		{
			for (PDAWorkOrderInfoConfig pdaWorkOrderInfoConfig : listPDAConfigInfo) {
				if(pdaWorkOrderInfoConfig.getContype().equals(IConfigEncoding.HARM_TYPE))//如果包含危害
				{
					WorkOrder workorder = (WorkOrder)UtilTools.
							judgeMapValue(mapParas, WorkOrder.class, true);
					checkHazardIsFinished(workorder);
				}
				if(pdaWorkOrderInfoConfig.getContype().equals(IConfigEncoding.PPE_TYPE))
				{
					WorkOrder workorder = (WorkOrder)UtilTools.
							judgeMapValue(mapParas, WorkOrder.class, true);
					checkPpeIsFinished(workorder);
				}
				if(pdaWorkOrderInfoConfig.getContype().equals(IConfigEncoding.MEASURE_TYPE))
				{
					WorkOrder workorder = (WorkOrder)UtilTools.
							judgeMapValue(mapParas, WorkOrder.class, true);
					checkMeasureIsFinished(workorder);
				}
				if(pdaWorkOrderInfoConfig.getContype().equals(IConfigEncoding.ENERGY_TYPE))
				{
					WorkOrder workorder = (WorkOrder)UtilTools.
							judgeMapValue(mapParas, WorkOrder.class, true);
	...
	private void checkHazardIsFinished(WorkOrder workorder) throws AppException {
		...
		if(cssavefied==null||!cssavefied.contains(String.valueOf(IConfigEncoding.HARM_TYPE)))
		{
	...
	private void checkPpeIsFinished(WorkOrder workorder) throws AppException {
		String cssavefied = "";
		cssavefied = workorder.getCssavefied();
		if(cssavefied==null||!cssavefied.contains(String.valueOf(IConfigEncoding.PPE_TYPE)))
		{
	 ...
	 * @throws AppException
	 */
	private void checkMeasureIsFinished(WorkOrder workorder) throws AppException {
		String cssavefied = "";
		cssavefied = workorder.getCssavefied();
		if(cssavefied==null||!cssavefied.contains(String.valueOf(IConfigEncoding.MEASURE_TYPE)))
		{
	...
	private void checkEnergyIsFinished(WorkOrder workorder) throws AppException {
		String cssavefied = "";
		cssavefied = workorder.getCssavefied();
		if(cssavefied==null||!cssavefied.contains(String.valueOf(IConfigEncoding.ENERGY_TYPE)))
		{
