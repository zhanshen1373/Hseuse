...
/**
 * ClassName: MergeCheckWorkToTask (小票签批时，大票必须是签批状态（服务器端验证）)<br/>
 * date: 2015年9月6日 <br/>
 * 
 * @author lxf
 * @version
...
public class MergeCheckWorkToTask extends AbstractCheckListener {
	public Object action(String action, Object... args) throws HDException {
		...
		//合并审批没有控制到具体的大小票关系
		boolean relationAction=isRelationActionType(IRelativeEncoding.BIGANDSPECIAL);
		if (relationAction) {
	...
	/**
	 * relationCheckWorkSign:(关联票验证). <br/>
	 * date: 2015年9月6日 <br/>
	 * 
	 * @author lxf
	 * @param workorder
	...
	private void relationCheckWorkSign(WorkOrder workorder, String strid)
		...
		if (!zypclass.equalsIgnoreCase(IWorkOrderZypClass.ZYPCLASS_ZYDP)) {
			// 表示当前票不是大票
			// 首先判断合并审批的票中是否包含大票
			if (strid.contains("'" + parent_id + "'")) {
				// 表示包含大票
				return;
			}
			StringBuilder sbsql = new StringBuilder();
			sbsql.append(
					"select dhzy_id,status,sjendtime from ud_zyxk_zysq where ud_zyxk_zysqid ='")
					.append(parent_id).append("' order by sjendtime desc");
			List<Map<String, Object>> list = getListMapResult(sbsql.toString());
