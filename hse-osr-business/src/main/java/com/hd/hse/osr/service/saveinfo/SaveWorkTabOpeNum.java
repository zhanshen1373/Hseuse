	...
	private void saveWorkTabOpeNum(WorkOrder workOrder,
		...
		try {
			if (workConfig.getContype() == IConfigEncoding.MEASURE_TYPE) {
				// 走措施
				if (workOrder.getCsnum() == null || (workOrder.getCsnum() == 0 && StringUtils.isEmpty(workOrder.getCssavenum()))) {
				...
				if (workConfig.getConlevel() != null
					...
					// 走逐条或者逐条批量
					String cstype = workConfig.getCstype() == null ? "" : workConfig
							.getCstype();
					String newcstype = UtilTools.convertToSqlString(cstype);
					String sql = "select isconfirm from ud_zyxk_zycsfcline where zycsfcnum in ( select zycsfcnum from ud_zyxk_zycsfc where "
							+ " pagetype='"+IConfigEncoding.FC+"' and status!='"+IWorkOrderStatus.REVIEW_STATUS_FINISH+ "' and datasource='PDA' and ud_zyxk_zysqid='"
