...
import com.hd.hse.dc.business.web.zyxk.GainCarXkzDataList;
...
public class GetCarXkzListActionListener extends AbstractActionListener {

    private static Logger logger = LogUtils
            .getLogger(GetCarXkzListActionListener.class);
    private List<SuperEntity> listsuper = null;
    // 作业票下载浏览标记
    private boolean browseFlag = false;

    public GetCarXkzListActionListener(boolean typeBoolean) {
        browseFlag = typeBoolean;
    };

    public GetCarXkzListActionListener() {
    ...
    private List<SuperEntity> getDownList(Object... args) {
        ...
        GainCarXkzDataList carXkzDataList=new GainCarXkzDataList();
        try {

            if (args != null && args.length > 0) {
                carXkzDataList.setSearch(args);
            }
            listsuper = (List<SuperEntity>) carXkzDataList.action("", args);
