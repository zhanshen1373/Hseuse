...
public abstract class UpListener extends AbsWebListener {
		...
		try {
			...
			if (null != listfile && listfile.size() > 0) {
				...
				if (null != listsuccess && null != listfile
					...
					// 表示上传图片报错
					this.sendMessage(IMessageWhat.ERROR, 9, 9, "上传照片成功："
							+ listsuccess.size() + ";失败："
							+ (listfile.size() - listsuccess.size()), response);
				}
			}
	...
	public abstract void beforeUpdateDeal() throws HDException;
